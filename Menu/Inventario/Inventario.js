...
function calculateInventoryMetrics() {
    ...
    // 6. Atualiza os campos de saída com formatação correta (Vírgula para exibição)
    const formatValue = (val) => formatToTwoDecimals(val).replace('.', ',');

    // 🌟 Formatador Inteligente para Porcentagem (%)
    const formatPercentSmart = (val) => {
        const num = parseFloat(val);
        if (isNaN(num) || !isFinite(num)) return '0,00';

        // REGRA DE OURO:
        // Se o número for >= 99.990 E estritamente MENOR que 100, usamos 3 casas.
        // Isso pega casos como 99.9984 (que antes virava 100.00) e transforma em 99.998
        if (num >= 99.990 && num < 100) {
            return num.toFixed(3).replace('.', ',');
        }

        // Caso contrário (ex: 100.00 exato ou 98.50), arredonda para 2 casas
        return (Math.round(num * 100) / 100).toFixed(2).replace('.', ',');
    };

    // Aplica formatação de DINHEIRO (R$)
    ...
    // Aplica formatação INTELIGENTE (%) nos campos de porcentagem
    if (netPercentInput) netPercentInput.value = formatPercentSmart(netPercent);
    if (grossPercentInput) grossPercentInput.value = formatPercentSmart(grossPercent);

    // Aplica também no Accuracy Item
    if (accuracyItemInput) accuracyItemInput.value = formatPercentSmart(accuracyItem);
}
...
async function searchAndLoadMonthlyData() {
    ...
    if (monthlyData) {
        ...
        // 🚨 PASSO IMPORTANTE: Dispara o cálculo!
        // Como os valores R$ estão carregados, a função calculateInventoryMetrics vai rodar
        // e usar a NOVA regra de 3 casas. Se o resultado for 99.9984, ela vai atualizar o input
        // para "99,998" ANTES de você salvar.
...
async function saveMonthlyData(e) {
    ...
    // Se o cálculo deu 99,998, o input está com "99,998"
    calculateInventoryMetrics();
    ...
    // 💡 NOVO: Captura o nome do contrato selecionado
    const contractName = monthlyContractNameInput ? monthlyContractNameInput.value : null;

    if (!contractId || !mesReferencia || !contractName) {
        displayMessage(monthlyFormMessage, "Erro: ID do Contrato, Mês de Referência ou Nome do Contrato faltando.", false);
        return;
    }
    ...
    const dataToSave = {
        ...
        // 💡 NOVO: Inclui o nome do contrato na carga de dados
        ...
        // Campos Calculados (R$ e %) - Lidos do INPUT.
        // Como o calculateInventoryMetrics colocou "99,998" no input, o parseInputFloat lerá 99.998
        // E salvará corretamente no banco!
